...
class Writer extends React.Component{
	...
	componentDidMount(){
        ...
        dispatch({type:'hotWriter/getWriterDetail',payload:{writerId:writerId}}).then(data=>{
        	if(data.code=='0'){
        		this.setState({bookMsg:data.data.bookMsg,selfMsg:data.data.selfMsg})
        	}else{
	...
	render(){
		...
		return(
			...
			{sessionStorage.getItem('userId')&&<Button type="primary" onClick={()=>sendInvite(item.id)}>添加好友</Button>}
			<Row gutter={16}>
				<Col span={8}> 
